...
public class PlayerMovement : MonoBehaviour
{
    ...
    private int dimensionToTeleport;
    private bool hasToJump = false;

    public bool HasToTeleport() { return hasToJump; }
    ...
    private void Movement()
    {
        ...
        if (movement > 0f)
        { // daca se misca la dreapta(a.k.a. inspre x pozitiv)
        ...
        { // daca se misca la stanga( x negativ)
            rigidBody.velocity = new Vector2(movement * speed, rigidBody.velocity.y);
    ...
    void TeleportPlayer()
    {
        transform.position = new Vector3(transform.position.x + LevelDistance * (dimensionToTeleport - LastSwap), transform.position.y, transform.position.z);
        camera.GetComponent<CameraPostProcessingColor>().moveToDimension(dimensionToTeleport.ToString(), lastPosition);
        LastSwap = dimensionToTeleport;
    }
    ...
    void Update()
    {
        ...
        {
            dimensionToTeleport = 1;
            hasToTeleport = true;
        }
        if (Input.GetKeyDown("2"))
        {
            dimensionToTeleport = 2;
            hasToTeleport = true;
        }
        if (Input.GetKeyDown("3"))
        {
            dimensionToTeleport = 3;
            hasToTeleport = true;
        }
        if (Input.GetKeyDown("4"))
        {
            dimensionToTeleport = 4;
            hasToTeleport = true;
        }
        if (Input.GetKeyDown("5"))
        {
            dimensionToTeleport = 5;
            hasToTeleport = true;
        }
